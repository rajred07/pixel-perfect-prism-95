...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const filteredMovies = useMemo(() => {
    ...
    return filtered.sort((a, b) => {
      ...
      if (aTitle === search && bTitle !== search) return -1;
      if (bTitle === search && aTitle !== search) return 1;
      if (aTitle.startsWith(search) && !bTitle.startsWith(search)) return -1;
      if (bTitle.startsWith(search) && !aTitle.startsWith(search)) return 1;
      
      return aTitle.localeCompare(bTitle);
    });
  }, [movies, searchTerm]);

  // Function to highlight search term in movie title
  const highlightSearchTerm = (title: string, searchTerm: string) => {
    ...
    return parts.map((part, index) => 
      regex.test(part) ? (
        <span key={index} className="bg-yellow-300 dark:bg-yellow-600 text-black dark:text-white font-semibold">
          {part}
        </span>
      ) : (
        part
