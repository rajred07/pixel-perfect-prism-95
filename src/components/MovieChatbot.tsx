...
export const MovieChatbot: React.FC = () => {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      message:
        "Hello! How can I help you today? Select a category from the top navigation and choose a movie to get started.",
      sender: "bot",
      timestamp: new Date().toISOString(),
    },
  ]);
  ...
  const handleSendMessage = (message: string, sender: "user" | "bot") => {
    ...
    setMessages((prev) => {
      ...
      if (updatedMessages.length > 2) {
        if (sender === "bot") {
          // Save only after bot responds
          if (selectedMovie) {
            saveChatSession(updatedMessages, `Chat about ${selectedMovie}`);
          } else if (selectedTopCategory) {
            saveChatSession(
              updatedMessages,
              `${
                selectedTopCategory.charAt(0).toUpperCase() +
                selectedTopCategory.slice(1)
              } Chat`
            );
          } else {
            saveChatSession(
  ...
  const handleMovieSelect = (movieTitle: string) => {
    ...
    const movieMessage: Message = {
      id: Date.now().toString(),
      message: `Great choice! I've set "${movieTitle}" as our current discussion topic. Feel free to ask me anything about this ${
        categoryLabels[selectedTopCategory as keyof typeof categoryLabels]
      } title - plot, characters, reviews, or any other questions you might have!`,
      sender: "bot",
      timestamp: new Date().toISOString(),
    };
    setMessages((prev) => [...prev, movieMessage]);
    ...
    <div className="flex w-full h-screen bg-white dark:bg-[#171212] max-w-7xl mx-auto">
      <Sidebar
        ...
        currentCategoryLabel={
          selectedTopCategory ? getCurrentCategoryLabel() : ""
        }
        loading={
          [
            "bollywood",
      ...
      <div className="flex h-full flex-col flex-1 min-w-0">
        ...
        {selectedCategory !== "chat-history" && (
          <MessageInput
            onSendMessage={handleSendMessage} // Updated to pass new signature
            placeholder="Type your message here..."
            selectedCategory={selectedTopCategory}
            selectedMovie={selectedMovie || ""}
          />
...
//   const pendingSaveRef = useRef<(() => void) | null>(null); // Defer Firebase writes
...
//               saveChatSession(updatedMessages, `Chat about ${selectedMovie}`);
//             } else if (selectedTopCategory) {
//               saveChatSession(updatedMessages, `${selectedTopCategory.charAt(0).toUpperCase() + selectedTopCategory.slice(1)} Chat`);
//             } else {
//               saveChatSession(updatedMessages, `General Chat - ${new Date().toLocaleDateString()}`);
//             }
//           };
//         }
//       }
...
//   const streamMessage = (fullMessage: string, sender: 'user' | 'bot') => {
...
//     setMessages((prev) => [
//       ...prev,
//       { id: messageId, message: '', sender, timestamp: new Date().toISOString() },
//     ]);

//     let currentMessage = '';
//     let index = 0;
//     const batchSize = 5; // Batch 5 characters at a time
//     let charBatch = '';

//     const interval = setInterval(() => {
//       if (index < fullMessage.length) {
//         charBatch += fullMessage[index];
//         index++;

//         // Update state every batchSize characters
//         if (charBatch.length >= batchSize || index === fullMessage.length) {
//           currentMessage += charBatch;
//           setMessages((prev) =>
//             prev.map((msg) =>
//               msg.id === messageId ? { ...msg, message: currentMessage } : msg
//             )
//           );
//           charBatch = ''; // Reset batch
//         }
//       } else {
...
//         // Execute deferred Firebase save
//         if (pendingSaveRef.current) {
//           pendingSaveRef.current();
//           pendingSaveRef.current = null;
//         }
//       }
//     }, 50); // 30ms delay per character
...
//     const movieMessage: Message = {
//       id: Date.now().toString(),
//       message: `Great choice! I've set "${movieTitle}" as our current discussion topic. Feel free to ask me anything about this ${categoryLabels[selectedTopCategory as keyof typeof categoryLabels]} title - plot, characters, reviews, or any other questions you might have!`,
//       sender: 'bot',
//       timestamp: new Date().toISOString(),
//     };
//     setMessages((prev) => [...prev, movieMessage]);
...
//     if (
//       selectedTopCategory &&
//       ['bollywood', 'hollywood', 'anime', 'dramas', 'kmovies', 'manga'].includes(
//         selectedTopCategory
//       )
//     ) {
//       if (firebaseMovies.length > 0) {
...
//     return <ChatArea messages={messages} isBotTyping={isBotTyping} />;
//   };
...
//       <div className="flex h-full flex-col flex-1 min-w-0">
//         <ChatHeader selectedMovie={selectedMovie} onSettingsClick={handleSettingsClick} />
...
//         <div className="flex-1 overflow-hidden min-h-0">{renderMainContent()}</div>
